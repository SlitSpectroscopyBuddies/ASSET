...
easily defined by the user to adapt the estimation process to the instrument and the observations. Fitting of the background, of the instrument's PSF and refinement of the spatio-spectral calibration of the detector are already possible if wanted.
...
Here, we present the quality of [`ASSET`](https://github.com/SlitSpectroscopyBuddies/ASSET) extracted spectra compared to the JWST pipeline extractions using a reference spectrum [@Bohlin:2014], resampled to the same resolution (\autoref{fig:NIRSpecSpectra} and \autoref{fig:MIRISpectra}). The target used for this comparison is the G dwarf reference star GSPC P 330 E, observed with JWST/NIRSpec and JWST/MIRI instruments (Program ID 1538). NIRSpec's data were observed the 08/30/2022 with the S1600A1 Fixed Slit, PRISM grating, CLEAR filter, and a 5 dithers pattern. MIRI's data were observed the 08/14/2022 with the MIRI LRS Slit, P750L filter, and a 2 dithers pattern. We also present the fitted chromatic PSF models at different wavelength ( \autoref{fig:NIRSpecPSFs} and \autoref{fig:MIRIPSFs}). We present results with both`ParametricPSF` (Gaussian, Moffat) and `NonParametricPSF` (series expansion). 

For NIRSpec (\autoref{fig:NIRSpecSpectra}),  `ASSET` results are more robust to the outliers than the pipeline extractions. For MIRI LRS (\autoref{fig:MIRISpectra}), the slope of `ASSET` spectra is more accurate above 5 $\mu$m as it is more robust to the background brightness than the pipeline method. Yet, it remains perfectible, especially under 5 $\mu$m. 
